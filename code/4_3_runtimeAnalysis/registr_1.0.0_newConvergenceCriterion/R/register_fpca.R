#' Register curves using constrained optimization and GFPCA WITH THE NEW VERSION OF THE CONVERGENCE CRITERION
#'
...
register_fpca <- function(Y, Kt = 8, Kh = 4, family = "binomial", max_iterations = 10, 
  ...
  time_warps[[2]] = registr_step$Y$index_scaled # registr_step$Y$index
  ...
  while( iter == 1 ||
         (iter < max_iterations && delta_index[iter] > convergence_threshold )){ # && error[iter] > 0.01 ){
  	...
  	delta_index[iter + 1] = mean((time_warps[[iter + 2]] - time_warps[[iter + 1]])^2) 
  	## calculate error
