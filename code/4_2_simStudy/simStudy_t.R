...
if (!checkmate::test_directory_exists("results_registry_t")) {
  reg <- makeExperimentRegistry(file.dir = "results_registry_t",
                                packages = c("registr"),
                                seed     = 2020)
}

reg <- loadRegistry("results_registry_t", writeable = TRUE)
...
saveRDS(pars, file = paste0("../../results/simStudy_",sim_type,"_t_pars.rds"))
res <- reduceResultsDataTable(ids=findDone(ids))
saveRDS(res, file = paste0("../../results/simStudy_",sim_type,"_t.rds"))
